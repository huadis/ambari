...
class LinuxDBMSConfig(DBMSConfig):
  ...
  def _setup_remote_db(self):
    setup_msg = (
      "Before starting Ambari Server, you must run the following DDL "
...
class PGConfig(LinuxDBMSConfig):
  ...
  def _configure_postgres(self):
    ...
    PGConfig._configure_pg_hba_ambaridb_users(
  ...
  def _change_db_files_owner(self):
    ...
    if not self._change_tables_owner():
      print_error_msg(
        """Ambari is unable to change ownership of the database tables in {database} to {user}.
...
class MySQLConfig(LinuxDBMSConfig):
  ...
  def _get_remote_script_line(self, scriptFile):
    MYSQL_INIT_SCRIPT = AmbariPath.get(
      "/var/lib/ambari-server/resources/Ambari-DDL-MySQL-CREATE.sql"
    )
...
class SQLAConfig(LinuxDBMSConfig):
  ...
  def __init__(self, options, properties, storage_type):
    ...
    self.server_name = DBMSConfig._init_member_with_prop_default(
      options, "sqla_server_name", properties, JDBC_SQLA_SERVER_NAME, "ambari"
    )
