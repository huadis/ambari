...
class DorisServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def getServiceConfigurationRecommendations(
        ...
        recommender.recommendDorisConfigurationsFromHDP32(
...
class DorisRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendDorisConfigurationsFromHDP32(
        self, configurations, clusterData, services, hosts
    ):
        """
        Recommend configurations for this service based on HDP 3.2.
        """
        putDorisRangerPluginProperty = self.putProperty(
            configurations, "ranger-doris-plugin-properties", services
        )

        # Recommend Ranger Hive authorization as per Ranger Hive plugin property
        if (
            "ranger-env" in services["configurations"]
            and "doris-env" in services["configurations"]
            and "ranger-doris-plugin-enabled"
            in services["configurations"]["ranger-env"]["properties"]
        ):
            rangerEnvDorisPluginProperty = services["configurations"]["ranger-env"][
                "properties"
            ]["ranger-doris-plugin-enabled"]
            putDorisRangerPluginProperty(
                "ranger-doris-plugin-enabled", rangerEnvDorisPluginProperty
            )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [
            (
                ...
                self.validateDorisRangerPluginConfigurationsFromHDP33,
            )
        ]

    def validateDorisRangerPluginConfigurationsFromHDP33(
        self, properties, recommendedDefaults, configurations, services, hosts
    ):
        validationItems = []
        ranger_plugin_properties = self.getSiteProperties(
            configurations, "ranger-doris-plugin-properties"
        )
        ranger_plugin_enabled = (
            ranger_plugin_properties["ranger-doris-plugin-enabled"]
        ...
        if "RANGER" in servicesList and ranger_plugin_enabled.lower() == "yes":
            ...
            if (
                not ranger_env
                or not "ranger-doris-plugin-enabled" in ranger_env
                or ranger_env["ranger-doris-plugin-enabled"].lower() != "yes"
            ):
                validationItems.append(
                    {
                        "config-name": "ranger-doris-plugin-enabled",
        ...
        return self.toConfigurationValidationProblems(
            validationItems, "ranger-doris-plugin-properties"
        )
