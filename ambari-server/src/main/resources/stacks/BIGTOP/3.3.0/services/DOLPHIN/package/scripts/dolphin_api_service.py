...
class DolphinApiService(Script):
    ...
    def start(self, env, upgrade_type=None):
        ...
        ).format(status_params.dolphin_api_server_pidfile)

        start_cmd = format(
            "sh " + params.dolphin_bin_dir + "/dolphinscheduler-daemon.sh start api-server"
        )
        Execute(
            start_cmd,
    ...
    def stop(self, env, upgrade_type=None):
        ...
        env.set_params(params)
        stop_cmd = format(
            "sh " + params.dolphin_bin_dir + "/dolphinscheduler-daemon.sh stop api-server"
        )
