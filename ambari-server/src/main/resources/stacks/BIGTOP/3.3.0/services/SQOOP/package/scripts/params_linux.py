...
SERVER_ROLE_DIRECTORY_MAP = {"SQOOP": "sqoop"}
...
stack_version_formatted = get_stack_version("sqoop")
...
if stack_version_formatted and check_stack_feature(
    ...
    sqoop_lib = f"{stack_root}/{stack_version_formatted}/usr/lib/sqoop/lib"
    hadoop_home = stack_select.get_hadoop_dir("home")
    hbase_home = f"{stack_root}/current/hbase-client"
    hive_home = f"{stack_root}/current/hive-client"
    sqoop_bin_dir = f"{stack_root}/current/sqoop/bin/"
    zoo_conf_dir = f"{stack_root}/current/zookeeper-client/conf"
    ...
    for driver_name in sqoop_jdbc_drivers:
        ...
        if driver_name and not driver_name == "":
            if driver_name == "com.microsoft.sqlserver.jdbc.SQLServerDriver":
                ...
                previous_jdbc_jar_name = default(
                    "/ambariLevelParams/previous_custom_hsqldb_jdbc_name", None
                )
                jdbc_driver_name = "hsqldb"
            else:
                raise Fail(format("JDBC driver '{driver_name}' not supported."))
        else:
            continue
