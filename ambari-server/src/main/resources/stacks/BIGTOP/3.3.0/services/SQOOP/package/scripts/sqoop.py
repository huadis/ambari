...
def sqoop(type=None):
    ...
    File(
        os.path.join(params.sqoop_conf_dir, "sqoop-env.cmd"),
...
def sqoop(type=None):
    ...
    if params.enable_atlas_hook:
        atlas_hook_filepath = os.path.join(
            params.sqoop_conf_dir, params.atlas_hook_filename
        )
        setup_atlas_hook(
            SERVICE.SQOOP,
...
def jdbc_connector():
    ...
    for jar_name in params.sqoop_jdbc_drivers_dict:
        ...
        custom_connector_to_remove = format(
            "{sqoop_lib}/" + str(params.sqoop_jdbc_drivers_to_remove[jar_name])
        )
        ...
        try:
            ...
            error_string = format(
                "Could not download {driver_curl_source}\n\
