...
class KuduServiceAdvisor(service_advisor.ServiceAdvisor):
  ...
  def __init__(self, *args, **kwargs):
    self.as_super = super(KuduServiceAdvisor, self)
  ...
  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    ...
    recommender.recommendKuduConfigurationsFromHDP33(configurations, clusterData, services, hosts)
...
class KuduRecommender(service_advisor.ServiceAdvisor):
  """
  Kudu Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(KuduRecommender, self)
    self.as_super.__init__(*args, **kwargs)

  def recommendKuduConfigurationsFromHDP33(self, configurations, clusterData, services, hosts):
    """
    Recommend configurations for this service based on HDP 3.3.
    """
    putKuduRangerPluginProperty = self.putProperty(configurations, "ranger-kudu-plugin-properties", services)   
    putKuduEnvProperty = self.putProperty(configurations, "kudu-env", services)    

    # print(putKuduRangerPluginProperty)
    # Recommend Ranger Hive authorization as per Ranger Hive plugin property
    if "ranger-env" in services["configurations"] and "kudu-env" in services["configurations"] and \
        "ranger-kudu-plugin-enabled" in services["configurations"]["ranger-env"]["properties"]:
      rangerEnvKuduPluginProperty = services["configurations"]["ranger-env"]["properties"]["ranger-kudu-plugin-enabled"]
      
      # print(rangerEnvKuduPluginProperty)
      putKuduEnvProperty("enable_ranger",rangerEnvKuduPluginProperty)
      putKuduRangerPluginProperty("ranger-kudu-plugin-enabled", rangerEnvKuduPluginProperty)
