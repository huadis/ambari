...
class SqoopRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendSqoopConfigurationsFromHDP23(
        self, configurations, clusterData, services, hosts
    ):
        ...
        if (
            "sqoop-atlas-application.properties" in services["configurations"]
            and "enable.external.atlas.for.sqoop"
            in services["configurations"]["sqoop-atlas-application.properties"][
                "properties"
            ]
        ):
            enable_external_atlas_for_sqoop = (
                services["configurations"]["sqoop-atlas-application.properties"][
                    "properties"
                ]["enable.external.atlas.for.sqoop"].lower()
                == "true"
            )
        ...
        if (
            "sqoop-env" in configurations
            and "sqoop.atlas.hook" in configurations["sqoop-env"]["properties"]
        ):
            enable_atlas_hook = (
                configurations["sqoop-env"]["properties"]["sqoop.atlas.hook"] == "true"
            )
        elif (
            "sqoop-env" in services["configurations"]
            and "sqoop.atlas.hook"
            in services["configurations"]["sqoop-env"]["properties"]
        ):
            enable_atlas_hook = (
        ...
        if enable_atlas_hook:
            ...
            putSqoopSitePropertyAttribute(
                "sqoop.job.data.publish.class", "delete", "true"
            )
