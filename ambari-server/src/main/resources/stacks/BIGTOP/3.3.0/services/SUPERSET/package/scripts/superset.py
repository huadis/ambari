...
class Superset(Script):
    ...
    def configure(self, env, upgrade_type=None):
        ...
        if not os.path.exists(format("{params.superset_pid_dir}/superset-init.lock")):
            ...
            Execute(
                f"source {params.superset_env_dir}/bin/activate;source {params.superset_config_dir}/superset-env.sh; {params.superset_bin_dir}/superset db upgrade",
                user=params.superset_user,
            )
            Execute(
                format(
                    "source {params.superset_env_dir}/bin/activate;source {params.superset_config_dir}/superset-env.sh ; {params.superset_bin_dir}/superset fab create-admin --username '{params.superset_admin_user}' --password '{params.superset_admin_password!p}' --firstname '{params.superset_admin_firstname}' --lastname '{params.superset_admin_lastname}' --email '{params.superset_admin_email}'"
                ),
                user=params.superset_user,
            )
            Execute(
                format(
                    "source {params.superset_env_dir}/bin/activate;source {params.superset_config_dir}/superset-env.sh ; {params.superset_bin_dir}/superset init"
                ),
                user=params.superset_user,
            )
    ...
    def get_daemon_cmd(self, params=None, command=None):
        return format(
            "source {params.superset_env_dir}/bin/activate; source {params.superset_config_dir}/superset-env.sh ; {params.superset_bin_dir}/superset.sh {command}"
        )
