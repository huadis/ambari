...
def setup_ranger_doris(upgrade_type=None, service_name="doris_fe"):
    ...
    if params.enable_ranger_doris:
        ...
        if params.retryAble:
            Logger.info(
                "doris: Setup ranger: command retry enables thus retrying if ranger admin is down !"
            )
        else:
            Logger.info(
                "doris: Setup ranger: command retry not enabled thus skipping if ranger admin is down !"
            )
        ...
        if params.xa_audit_hdfs_is_enabled:
            try:
                ...
                Logger.exception(
                    "Audit directory creation in HDFS for doris Ranger plugin failed with error:\n{0}".format(
                        err
                    )
                )
        ...
        setup_ranger_plugin(
            ...
            params.java_home,
            params.repo_name,
            params.doris_ranger_plugin_repo,
            params.ranger_env,
            params.ranger_plugin_properties,
            params.policy_user,
            params.policymgr_mgr_url,
            params.enable_ranger_doris,
            conf_dict=params.doris_fe_conf_dir,
            component_user=params.doris_user,
            component_group=params.user_group,
            cache_service_list=["doris"],
            ...
            component_user_principal=params.doris_user_principal_name
            if params.security_enabled
            else None,
            component_user_keytab=params.doris_user_keytab
            if params.security_enabled
            else None,
        )
